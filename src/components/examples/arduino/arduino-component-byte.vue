<template>
  <div class="module twoColParent">
    <div class="twoColChild">
      <p>This component includes a simple example of arduino communications using bytes. Use it with the corresponding two_way_comms sketch found in the /src/components/examples/arlduino/sketch folder.</p>
...
const CMD_BITS = 0b11110000;
const PAYLOAD_BITS = 0b00001111;

const BUTTON_STATE = 0b00010000;
const RELEASE = 0b00000000;
const PRESS = 0b00000001;

const LED_STATE = 0b00100000;
const ON = 0b00000000;
const OFF = 0b00000001;
...
export default {
  ...
  async mounted() {
    ...
    if (isElectron()) {
      let { createByteArduino } = await import('@explo/arduino');
      this.arduino = createByteArduino(this.config.arduino);
  ...
  methods: {
    toggleLed() {
      if (isElectron()) {
        ...
        if (this.LEDon == true) {
          this.write(LED_STATE | ON);
        } else {
          this.write(LED_STATE | OFF);
        }
      }
    },
    parseArduinoData(byte, connection) {
      ...
      console.log('parseArduinoData', byte, connection);
      this.$parent.resetTimeout();

      const commandByte = byte & CMD_BITS;
      const payloadByte = byte & PAYLOAD_BITS;
