<template>
  <div class="module twoColParent">
    <div class="twoColChild">
      <p>This component includes a simple example of arduino communications using JSON. Use it with the corresponding two_way_comms sketch found in the /src/components/examples/arlduino/sketch folder.</p>
...
export default {
  ...
  async mounted() {
    ...
    if (isElectron()) {
      let { createJSONArduino } = await import('@explo/arduino');
      this.arduino = createJSONArduino(this.config.arduino);
      ...
      // this.arduino.open( {'usbProductId':32833, 'usbVendorId':9025} );
  ...
  methods: {
    ...
    parseArduinoData(message, connection) {
